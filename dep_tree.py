...
class DependencyTree:
    ...
    def third_level_dep_mapping(self):
        ...
        simple_dep_map={'TAM':'xcomp','VPP':'obl','PART':'mark','NPRT':'compound','NVE':'compound:lvc','ENC':'compound:lvc','LVP':'compound','NE':'compound','MESU':'nmod'}
        ...
        for idx in range(0,len(self.words)):
            ...
            if old_role=='OBJ': #mapping role of OBJ
                ...
                if len(head_obj2_children)>0:
                    ...
                    self.labels[idx]='obj'
                    rol_changed=True
            if old_role=='ADV' or old_role=='MOZ' or old_role=='ADVC' or old_role=='AJPP' or old_role=='NEZ':
                if old_pos=='ADV':
                    self.labels[idx]='advmod'
                    rol_changed=True
                elif old_pos=='NOUN' or old_pos=='PROPN':
