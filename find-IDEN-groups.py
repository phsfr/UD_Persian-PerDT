def find_IDEN_group(iden_tok,tok_dic,lin):
    ...
    while len(stack)!=0:
        ...
        for tok in tok_dic:
            #try:
            if tok_dic[tok][3].strip()==h_idx.strip() and tok_dic[tok][4].strip()!='APP' and tok_dic[tok][4].strip()!='NCONJ' and tok_dic[tok][1].strip() in possible_pos and tok_dic[tok][0].strip() not in forbid_words: #tok_dic[tok][0]!=':' and tok_dic[tok][0]!='،':#(tok_dic[tok][0]!='که' or tok_dic[tok][0]!=':' or tok_dic[tok][0]!='و'): #or tok_dic[tok][0]!='،':
                    stack.append(tok)
                    #break
            #except IndexError:
