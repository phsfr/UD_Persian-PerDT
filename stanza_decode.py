...
def parse(nlp, content):
	...
	for s in range(len(doc.sentences)):
		for word in doc.sentences[s].words:
			if '-' in str(word.id):
				tokens.append("\t".join([str(x) for x in [word.id, word.text, "_", "_", "_", word.misc, "_", "_", "_", "_"]]))
			else:
				tokens.append("\t".join([str(x) for x in [word.id, word.text, word.lemma, word.upos, word.xpos, word.misc, word.head, word.deprel, "_", "_"]]))
	return "\n".join(tokens)
