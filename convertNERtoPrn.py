...
def find_foreign(word):
    ...
    #not_allowed_chars=[',','×','ْ','٫']
    if word.isnumeric() or len(word)<=1 or word in except_words or hyphen_free.isnumeric() or dot_free.isnumeric() or slash_free.isnumeric() or quote_free.isnumeric()or cent_free.isnumeric():
        return False
    for ch in word:
        if ch=='\u200c':
            continue
...
def post_process(fr,fw):
    ...
    ghamari_months=['محرم','صفر','ربیع‌الاول','ربیع‌الثانی','جمادی‌الاول','جمادی‌الثانی','رجب','شعبان','رمضان','شوال','ذیقعده','ذیحجه']
    lines=fr.readlines()
    for line in lines:
